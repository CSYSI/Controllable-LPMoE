...
_base_ = [
    '../_base_/models/mask2former_uniperceiver.py',
    '../_base_/datasets/mydata_MIS_SC_ISIC2017.py',
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='Controllable_LPMoE_Uniper',
        patch_size=16,
        embed_dim=1024,
        depth=24,
        num_heads=16,
...
# dataset settings
img_norm_cfg = dict(
    mean=[181.83, 154.76, 143.16], std=[39.41, 41.73, 45.90], to_rgb=True)
img_norm_cfg_test = dict(
    mean=[176.55, 139.96, 128.79], std=[37.85, 37.44, 40.38], to_rgb=True)
...
evaluation = dict(interval=2000, metric=['mIoU','mFscore','mDice'], save_best='mIoU', rule='greater')
